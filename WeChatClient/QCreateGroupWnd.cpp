...
void QCreateGroupWnd::updateData()
{
	...
	for (; itMap != itMapEnd; itMap++)
	{
		...
		{
			addListWnd1Item("./img/head2.png", itMap->first, 
				QString::number(itMap->first).toStdString().c_str(), itMap->second.toStdString().c_str());
		}
	}
...
void QCreateGroupWnd::slot_selRBtnClick(QMap<QString, QString> map)
{
	...
	if (map["isSel"] == "true") {
		if (hasThisWndByRolename(map["rolename"], m_listWnd2) == false) {
			addListWnd2Item(map["headimg"].toLocal8Bit().data(),map["friendid"].toInt(),
